...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, num_classes=10):
        ...
        self.layer1 = self._make_layer(block, 64, num_blocks[0], stride=1, conv_kernel=3, shortcut_kernel=1, pad=1)
        self.layer2 = self._make_layer(block, 128, num_blocks[1], stride=2, conv_kernel=3, shortcut_kernel=1, pad=1)
        self.layer3 = self._make_layer(block, 256, num_blocks[2], stride=2, conv_kernel=3, shortcut_kernel=1, pad=1)
        self.layer4 = self._make_layer(block, 512, num_blocks[3], stride=2, conv_kernel=3, shortcut_kernel=1, pad=1)
